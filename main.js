...
if (!opts['test']) {
  if (global.db) setInterval(async () => {
    ...
    if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', global.authFileJB], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))
  }, 30 * 1000)
}
...
global.reload = async (_ev, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (filename in global.plugins) {
      if (existsSync(dir)) conn.logger.info(` SE ACTUALIZÓ - '${filename}' CON ÉXITO`)
      else {
        conn.logger.warn(`SE ELIMINÓ UN ARCHIVO : '${filename}'`)
        return delete global.plugins[filename]
      }
    } else conn.logger.info(`SE DETECTÓ UN NUEVO PLUGINS : '${filename}'`)
    const err = syntaxerror(readFileSync(dir), filename, {
      sourceType: 'module',
      allowAwaitOutsideFunction: true,
    })
    if (err) conn.logger.error(`SE DETECTÓ UN ERROR DE SINTAXIS | SYNTAX ERROR WHILE LOADING '${filename}'\n${format(err)}`)
    else {
      try {
